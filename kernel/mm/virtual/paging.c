...
void paging_init()
{
	...
	debug_module(PAGING,
				 "[PAGING] Directory at %p, Table[0] at %p\n",
				 page_directory,
				 first_page_table);
	debug_module(PAGING,
				 "\n==================================================\n");
...
//
void paging_map_page(uint32_t virtual_addr,
					 ...
					 uint32_t flags)
{
	...
	else
	{
		...
		//  pmm_alloc_frame returns the physical address of the allocated frame
		//  (page table)
		...
		page_directory[pdir_index] =
			((uint32_t)page_table) | PAGE_PRESENT | PAGE_WRITE;
	}

	//  Now set the page table entry to point to the physical frame with given
	//  flags
	...
	//  Flush TLB for new page, rest unchanged (to ensure hardware picks up new
	//  mapping)
