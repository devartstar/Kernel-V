...
IDT (Interrupt Descriptor Table):
    ...
    - CPU only stores the base address + limit in the IDTR (Interrupt Descriptor
Table Register).

When an interrupt occurs:
    CPU accesses the IDT entry using the base address and interrupt vector
number.
*/
...
typedef struct {
    ...
    uint8_t always0;   //  Reserved bits, this byte must always be zero
    ...
    Flags[8 bits]: Present, DPL, Gate Type
        ...
        Bits 5-6 (DPL)  :   Descriptor Privilege Level Bit
                            (0 = Kernel, 3 = User)
                            Min privilege required to trigger interrupt via INT
    n (Software Interrupt) Ignored for hardware interrupts
        ...
        Bit 0-3 (Type)  :   Gate Type Bit
                            ...
                            0x5     - Task Gate (Instead of jumping to ISR, it
    switches task) 0x6|0xE - Interrupt Gate (Jumps to ISR, disables hardware
    interrupts) 0x7|0xF - Trap Gate (Jumps to ISR, does not disable hardware
    interrupts) 0x6, 0x7 are for 16 bits | 0xE, 0xF are for 32 bits
    */
    uint8_t flags;
