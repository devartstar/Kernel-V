 ...
 */
void set_panik_mode(panik_mode_t mode) { current_panik_mode = mode; }

/**
 * Get current panic mode
 */
panik_mode_t get_panik_mode(void) { return current_panik_mode; }

/**
 * Get panic state for testing
 */
const panik_state_t *get_panik_state(void) { return &panik_state; }
...
void panik(const char *fmt, ...) {
    ...
    va_list args;
    va_start(args, fmt);
    int len = my_vsnprintf(panik_state.last_panik_msg,
                           sizeof(panik_state.last_panik_msg), fmt, args);
    va_end(args);
    ...
    vga_print_string(panik_state.last_panik_msg,
