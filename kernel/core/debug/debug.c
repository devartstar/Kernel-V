...
void debug_gdt_entry(int num) {
    ...
    uint32_t base = (gdt[num].base_high << 24) | (gdt[num].base_middle << 16) |
                    gdt[num].base_low;
    uint32_t limit = ((gdt[num].granularity & 0x0F) << 16) | gdt[num].limit_low;
    /*
     * If G (granularity) bit in Flags is 1 => 1 Byte granularity
     * Else it is 4Kb (2^12 Byte) granularity
     */
    if (gdt[num].granularity & 0x80) {
        limit = (limit << 12) | 0xFFF;
    }
    uint8_t access = gdt[num].access;
    uint8_t flags = gdt[num].granularity;
    uint8_t present = (access >> 7) & 0x1; /* Present Bit */
    uint8_t dpl = (access >> 5) & 0x3;     /* Display Privilege Level */
    uint8_t s =
        (access >> 4) & 0x1; /* Descriptor bit - 0 for TSS, 1 for Code/Data */
    const char *type = "---";
    if (present) {
        if (s) {
            type = (access & 0x8)
                       ? "Code"
                       : "Data"; /* Executable Bit - 0 for Data, 1 for Code */
        } else {
            type = "Sys";
        }
    }
    ...
    debug_module(
        IDT_GDT,
        "%3d | 0x%02x | 0x%08x | 0x%08x | 0x%02x | 0x%02x |  %d  | %d | %s\n",
        num, num << 3, base, limit, access, flags, dpl, present, type);
}

void debug_gdt_table() {
    debug_module(IDT_GDT, "GDT TABLE:\n");
    debug_module(IDT_GDT, "Idx | Sel  | Base       | Limit      | Acc  | Flg  "
                          "| DPL | P | Type\n");
...
void debug_print_esp_args(uint32_t arg1, uint32_t arg2) {
    debug_module(STACK_HEAP,
                 ...
                 PRINT_UINT32(arg1), PRINT_UINT32(arg2));
}

void debug_e820_map(void) { print_e820_map(); }
