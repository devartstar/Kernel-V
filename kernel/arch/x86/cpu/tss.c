 ...
 * CPU is running (esp) -> Someone assign faulty address to esp -> Timer
 * Interrupt fires -> Kernel tries to save CPU context at esp -> Page Fault
 * occurs because esp is invalid -> Usual Page Faults (kernel tries self
 * recovery) -> Saves CPU context before recovery -> Still trying to push at bad
 * esp -> Double Fault occurs -> Fallback to TSS defined stack.
 */
...
void init_tss() {
    ...
    /* CR3 - CPU Control Register 3 stores the physical address of the page
       directory Read current CR3 value - THIS WILL BE ZERO because paging is
       not yet enabled */
    ...
    /* per process kernel stack - on context switch */
    /* [todo] tss.esp0 - should be updated to current process kernel stack */
    tss_df.esp0 = (uint32_t)(double_fault_stack + DOUBLE_FAULT_STACK_SIZE -
                             16); // Use same stack as DF handler
    ...
    debug_module(TSS,
                 "Initialized double fault TSS with ESP0=0x%08x SS0=0x%04x\n",
