...
void page_fault_handler (page_fault_stack_t* frame)
{
    ...
    debug_module (PAGING, "[PAGE FAULT] at address: 0x%x, error code: 0x%x [eip=0x%x, esp=0x%x, ebp=0x%x]\n", 
            fault_address, 
            frame->error_code, 
            frame->eip,
            esp,
    ...
    {
        debug_module (PAGING, "[PAGE FAULT] Address within kernel heap region: allocating and mapping new page.\n");
    ...
    if (fault_address >= KERNEL_STACK_BOTTOM_VIRT + PAGE_SIZE && fault_address < KERNEL_STACK_TOP_VIRT) {
        if (fault_address >= frame->esp - STACK_GROWTH_GAP && fault_address < frame->esp) {
            debug_module (PAGING, "[PAGE FAULT] Stack growth: mapping new stack page at 0x%x (esp=0x%x)\n", fault_address, frame->esp);
            void* new_frame = pmm_alloc_frame();
